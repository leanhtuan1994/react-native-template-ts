...
Since we're using Uniwind (which follows TailwindCSS patterns) and expo-router, we let them handle the application of theme, and we just take care of the colors we want. We set the colors in `ui/theme/colors.js` and we use them in our hook `useThemeConfig.tsx` to get the theme object that we pass to ThemeProvider directly. For more information check out [expo-router](https://docs.expo.dev/router/appearance/).

### How do we handle theme changes?

We use the `loadSelectedTheme` function to load the theme from storage if there's a theme saved, otherwise we let Uniwind use the default theme (system). To set the selected theme, we use the `useSelectedTheme` hook, which sets the theme in storage and updates the color scheme of the app.
