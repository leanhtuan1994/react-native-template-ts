...
Most applications require some form of authentication. Atlas comes with a solid foundation for managing auth state so you can get started quickly without building it from scratch.

Authentication is global to the application, so Atlas uses **Zustand** to manage the authentication state.
...
A common pattern is to gate navigation behind authentication status. In Atlas, the root layout (`src/app/_layout.tsx`) reads `status` from the auth store and redirects accordingly.
