...
description: Complete guide to navigation using Expo Router with typed routes and route groups in Caracal.
---
...
**Available schemes:**
- `caracalApp://` — Custom scheme (all environments)
- `exp+caracalapp://` — Expo Go scheme (development)
...
**iOS Simulator:**
```bash
xcrun simctl openurl booted "caracalApp://settings"
```

**Android Emulator:**
```bash
adb shell am start -W -a android.intent.action.VIEW -d "caracalApp://settings"
```

**Expo Go:**
```bash
npx uri-scheme open "exp+caracalapp://components/button" --ios
npx uri-scheme open "exp+caracalapp://components/button" --android
```
...
3. Test with correct scheme: `caracalApp://` (not `http://`)
...
- Or use deep link: `caracalApp://components`
- Or see [Component Showcase documentation](/docs/ui-and-theme/component-showcase)

All showcase routes are listed in [Project Structure](/docs/getting-started/project-structure).
