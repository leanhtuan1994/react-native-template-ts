...
description: Complete guide to managing environment variables with Zod validation, type-checking, and multi-environment support in Caracal starter.
---
...
<Callout type="info">
This setup is inspired by [T3 Stack](https://create.t3.gg/)'s excellent environment variable system.
</Callout>
...
- Passed via `app.config.ts` → `extra` field
...
// Production:  com.caracal (no suffix)
```
...
<Callout type="info">
If you don't commit `.env` files to your repo (recommended for security), make sure to configure them in your CI/CD pipeline. See [App Releasing Process](/docs/ci-cd/app-releasing-process) for GitHub Actions setup.
</Callout>
...
const invalid = Env.DOES_NOT_EXIST;     // ❌ Type error
```
...
**Wrong type:**
```bash
❌ Invalid environment variables: { VAR_NUMBER: [ 'Expected number, received string' ] }
```
...
See [App Releasing Process](/docs/ci-cd/app-releasing-process) for complete CI/CD setup.
...
- **Type safety** - No `any` types, full autocomplete
