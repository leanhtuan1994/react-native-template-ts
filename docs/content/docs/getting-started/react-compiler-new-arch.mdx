...
description: Understanding the experimental features enabled in Caracal starter - React Compiler and New Architecture.
---
...
**With React Compiler (enabled in Caracal):**
```tsx
...
The legacy architecture was removed in Expo SDK 55, and the `newArchEnabled` option no longer exists. Caracal runs on Expo SDK 57 / React Native 0.86, so the New Architecture is always active. If a library does not support it, replace the library rather than trying to opt out — see [Library Compatibility](#library-compatibility).
