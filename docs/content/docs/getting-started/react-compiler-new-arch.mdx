...
<Callout type="warn">
**Experimental Status**: While both features are stable enough for production use in Expo SDK 56, they're still marked as experimental. The React Native and React teams are actively developing them, and APIs may evolve.
</Callout>
...
**A:** Partially. Expo Go in Expo SDK 56 has New Architecture enabled, but you need a custom dev client for full support with native modules. Use `pnpm prebuild` and `pnpm ios`/`pnpm android`.
